...
- Common harness written (`scripts/perf3.py`, `stab.py`, `burst.py`, `cachetest3.py`): prompts cut at the token, corpus offsets meant to be distinct (they were not across sizes with `--reps 1` — found on 18 Sept, see RESULTS correction), greedy, 200–300 tokens, client TTFT/throughput; stability = random sizes + a 4-stream burst every 6th iteration, seed 1.
...
- **VRAM 1 075 MHz** (Tamalero's idea, done in the driver instead of the ROM): module rebuilt, host rebooted, `OD_MCLK` exposed; `memtest_vulkan` 5 min 30 per card clean at 1 075, decode +3.9 %. We then widened the driver-side ceiling to 1 250 and screened upwards: card0 loses throughput at 1 125 (EDR retries), card2 is still clean at 1 150, card1 hangs at 1 150 — and our own script, by writing an OverDrive setting to the card that had just been reset, wedged its SMU (host reboot). Script fixed to stop dead on any GPU event; decision: **1 075 MHz on all three**, persistent through the existing undervolt service (one OverDrive commit per card). Details and the full table: RESULTS §2c.
- **Clean campaign at 1 075 MHz, 160 W then 200 W** (16:00–17:35, `scripts/host/campaign-1075.sh`, 13 runs, 0 GPU events) — and two corrections to what we had published. (1) `perf3.py --reps 1` started every prompt size at corpus offset 0, so cache-on prefill at ≥ 16K included hits: 130K is **1 933 tok/s, not 2 493** (2 254 at 200 W). (2) The quantised drafter's "123 wall at 8 streams, +60 %" compared `--max-num-seqs 4` with 8: like for like it is 88–90 (+17 %). Confirmed cleanly: 200 W = +13–17 % prefill and 0 on decode; 1 075 MHz = +4 % decode. Unexplained: no-MTP at 4 streams reads 139.5 on both caps against 157 in the morning. Tables in RESULTS §0.

## What is still open

1. Measure the 261K prefill with prefix caching off (the published 2 716 included hits; 130K is done: 1 933) and decode > 32K with server counters (the streaming client mis-measures MTP at long context).
